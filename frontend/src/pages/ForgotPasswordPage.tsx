...
export function ForgotPasswordPage() {
  ...
  const onSendCode = async () => {
    ...
    try {
      await sendForgotPasswordVerificationCode(e);
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setMessage(`重置密码失败: ${err?.response?.data?.message ?? err.message}`);
    }
  };
        ...
        <div>
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-stretch">
            <input
              ...
              placeholder="注册邮箱"
            />
            <SendCodeButton htmlType="button" onClick={onSendCode} disabled={cooldown > 0}>
